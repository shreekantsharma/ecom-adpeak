    ...
    if raw_df is not None:
        ...
        if len(date_range) == 2:
            current_df = current_df[
                (current_df['order_date'].dt.date >= date_range[0]) &
                (current_df['order_date'].dt.date <= date_range[1])
            ]

        # 2. Order Status (Global Options, Independent of Date/Payment for visibility, but applied immediately)
        # Strategy: To support strict cascading for *dependent* fields (Channel/SKU/Product), 
        # we apply filters sequentially.
        ...
        if selected_status != 'All':
            current_df = current_df[current_df['status_bucket'] == selected_status]
        ...
        if selected_payment != 'All':
             current_df = current_df[current_df['payment_method'].astype(str) == selected_payment]

        # 4. Channel (Dependent on Date, Status, Payment)
        # Show only channels available in the currently filtered data
        channel_options = ['All'] + sorted(current_df['channel'].astype(str).unique().tolist())
        selected_channel = st.sidebar.multiselect("Channel", channel_options, default=None)
        
        if selected_channel:
            current_df = current_df[current_df['channel'].isin(selected_channel)]

        # 5. SKU (Dependent on Channel + previous)
        sku_options = ['All'] + sorted(current_df['sku'].astype(str).unique().tolist())
        selected_sku = st.sidebar.multiselect("SKU", sku_options, default=None)
        
        if selected_sku:
            current_df = current_df[current_df['sku'].isin(selected_sku)]

        # 6. Product Name (Dependent on SKU + previous)
        product_options = ['All'] + sorted(current_df['product_name'].astype(str).unique().tolist())
        selected_product = st.sidebar.multiselect("Product Name", product_options, default=None)
        
        if selected_product:
             current_df = current_df[current_df['product_name'].isin(selected_product)]
